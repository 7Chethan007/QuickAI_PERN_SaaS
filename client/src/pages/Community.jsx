  ...
  const fetchCreations = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        return toast.error(data.message || 'Could not fetch creations')
      }
    } catch (error) {
  ...
  const imageLikeToggle = async (id) => {
    try {
      ...
      if (data.success) {
        ...
        return toast.error(data.message || 'Could not like the image')
      }
    } catch (error) {
      toast.error(error.response?.data?.message || 'Something went wrong. Please try again.')
    }
        ...
        rounded-lg'>
              ...
              <div className='flex gap-1 items-center'>
                ...
                <Heart onClick={() => imageLikeToggle(creation.id)}
                 className={`min-w-5 h-5 hover:scale-110 cursor-pointer ${creation.likes.includes(user.id) ? 'fill-red-500 text-red-600' :
                    'text-white'}`} />
              </div>
            </div>
          </div>
